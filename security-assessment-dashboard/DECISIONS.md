...
## Phase 10 — Tool Management 2.0 (Production Plugin Management)

### `diagnostics()` lives on `DetectionOnlyPlugin`, not as a new abstract method on `BasePlugin`

Every tool plugin that needs rich diagnostics (all 15 detection-only tools, plus Nmap, which also extends `DetectionOnlyPlugin` and only overrides the genuinely tool-specific methods) already shares that base class. Adding `diagnostics()` there — one real implementation reused by all sixteen — satisfies the phase's explicit "reusable diagnostics, no duplicated detection logic" requirement directly. Making it abstract on `BasePlugin` instead would have forced the two purely-internal, never-really-installed reference plugins (`example_plugin`, `dummy_execution`) to grow a diagnostics implementation they have no real use for, for no benefit — those exist to prove the plugin framework works, not to be diagnosed by Tool Management.

### `diagnostics()` and `health()` share one `_build_health()` helper instead of `diagnostics()` calling `health()`

The first working version of `diagnostics()` called `self.health()`, which internally calls `resolve_executable()` and `get_version()` again — meaning a single diagnostics fetch ran the executable-resolution and version-detection subprocess logic twice. Refactored so both `health()` and `diagnostics()` detect the executable/version exactly once each, then hand the same `(executable, version)` pair to a shared `_build_health()` for the actual health verdict. Same output, half the subprocess calls when only diagnostics is requested.

### Tolerant version detection falls back through flag conventions AND patterns, not just one or the other

The phase brief's complaint was that version detection was "too strict" — a plugin declares exactly one version flag (`VERSION_ARGS`) and one regex (`VERSION_PATTERN`), and if either is wrong for the actual installed version of that tool, `get_version()` silently returns `None` (reported as "degraded" health, indistinguishable from a genuinely broken install). `detect_version()` now tries the plugin's preferred flag first, then `-version`/`version`/`-v`/`-V` in turn; for whichever flag actually produces output, it tries the plugin's own pattern first, then a generic dotted-version regex. Getting the flag right OR the pattern right (not both) is now enough to detect a version. Verified live: Nmap's own declared `VERSION_ARGS = ["--version"]`/`VERSION_PATTERN` still matches first-try against the real installed nmap (v7.97) on this dev machine, so tolerant fallback logic doesn't change behavior for a plugin that's already correct — it only helps ones that aren't.

### `list_profiles()` returns `[]` for a tool without a profile system; every mutating profile endpoint still 422s

Before this phase, *every* Scan Profile endpoint 422'd for a tool without a `profile_manager` (14 of 15 supported tools). That was fine when Scan Profiles had no dedicated UI (a caller had to already know to check), but Tool Management 2.0's Profiles tab is generic — it's shown for every tool, and "this tool doesn't have Scan Profiles" needs to render as a normal empty state, not an error toast, for the common case. Changed only the read path (`list_profiles`/the `GET` list route); `create`/`update`/`delete`/`duplicate`/`import`/`enable`/`disable` still 422 immediately, since there's nothing to mutate for a tool with no profile system and silently no-op-ing there would be more confusing than a clear error. `test_scan_profiles_api.py`'s old `test_profiles_for_a_tool_without_profile_support_is_a_client_error` was split into a listing test (now asserts `200`/`[]`) and a mutating test (still asserts `422`).

### Profile enable/disable is a config flag (`disabled_profile_ids`), never a profile file edit

Disabling a *built-in* profile can't mean editing or deleting it — `ProfileManager.update_custom`/`delete_custom` both explicitly refuse to touch built-ins (a Phase 7 guarantee this phase doesn't relax). Modeled instead as a set of disabled ids on the same `PluginConfiguration`/`ToolConfiguration` pair every other tool setting already lives on (`disabled_profile_ids` in-memory, `disabled_profiles_json` persisted), toggled via `ToolService.set_profile_enabled()` and read by `ScanProfileService._to_read()` computing `enabled = profile.id not in registered.config.disabled_profile_ids`. Works identically for built-in and custom profiles, needs no new profile-manager method, and a disabled profile remains fully viewable/exportable/duplicable — disabling only ever means "don't offer this as a scan option," never "this doesn't exist."

### Real bug found and fixed: persisted `ToolConfiguration` never survived a backend restart until the user re-saved it

While wiring `disabled_profile_ids` to actually take effect without a reload, tracing `PluginConfiguration`'s lifecycle showed `_apply_to_live_config()` — the one function that pushes a DB `ToolConfiguration` row onto the live, in-memory object a running plugin instance actually reads — was called from exactly one place: `update_configuration()`, i.e. only right after a user submits the Settings form. `_sync_catalog()` (which runs on every `discover()` and is what actually creates/finds each tool's `ToolConfiguration` row) never called it. Consequence: any previously-saved custom executable path, proxy, timeout, rate limit, or wordlist silently stopped being honored the moment the backend process restarted — the database still showed it configured (`ToolDetail.configuration` reads straight from the DB row), but the live plugin object a scan would actually use was back to defaults, with no error or indication anywhere. Fixed by calling `_apply_to_live_config()` inside `_sync_catalog()` too. This is a general fix, not narrow to the new `disabled_profile_ids` field — every existing configuration field benefits, and it's the reason this phase's enable/disable feature survives a restart at all.

### Minimum/recommended tool version: only Nmap's manifest sets one, and only "minimum," not "recommended"

`PluginManifest` gained both fields, but populating them for all 15 tools with numbers this project has never actually version-tested against would be exactly the kind of fabricated claim `.claude/CLAUDE.md` forbids — "never generate placeholder data" applies to metadata the UI presents as fact, not only to scan results. Nmap got `minimum_tool_version: "7.00"` (a real, conservative floor — XML output and NSE scripting predate it by years, and it's the one tool this codebase has extensively tested against). No tool got a `recommended_tool_version`: "recommended" is inherently an opinion about a moving target (today's latest stable goes stale), not a documented fact like a minimum-supported floor, so leaving it unset for every tool for now is the honest choice.

### Manifests normalized to Linux-only `install_instructions`, and Nuclei's stale Windows-era entry rewritten

14 of 15 manifests gained real `install_instructions` (only `dirsearch` deliberately left blank, preserving `test_get_tool_detail_install_instructions_is_none_when_manifest_omits_it` as a genuine "field omitted" case rather than a synthetic one). Nuclei's pre-existing entry (written before the Linux-only runtime decision) had `windows`/`linux`/`macos` keys; rewritten to a single `linux` key, consistent with `.claude/CLAUDE.md`'s Linux-only rule, and the one test asserting its shape (`test_get_tool_detail_surfaces_install_instructions_when_declared`) updated to check `"linux"` instead of `"windows"`. All new instruction text is sourced from the same research already written up in `docs/LINUX_TOOL_INSTALLATION.md` (added during the Linux-only architectural change) rather than re-derived, so there's one place this information is actually researched and two places it's surfaced.

### Real bug found and fixed during Playwright verification: PATH-directory list used a non-unique React key

`ToolDiagnosticsPanel` rendered one `<Badge key={dir}>` per `PATH` entry. This dev machine's real `PATH` environment variable contains genuine duplicate directories (e.g. `C:\Program Files\Git\usr\bin` appears twice — a byproduct of how Git for Windows and other installers each prepend their own entries over time), which a synthetic test value would never have surfaced. A live React duplicate-key warning, caught only by rendering the real `GET /tools/{name}/diagnostics` response in a real browser — the same class of bug (and the same fix: qualify the key with the array index) as Phase 9's `DistributionBarChart` `<Cell key={row.label}>` collision, and further confirmation of this project's established practice that a page which type-checks and builds cleanly can still be wrong until it's actually rendered and looked at.

### `last_validated_at` is stamped only by an explicit validate action, not by viewing a tool's detail page

`_to_detail()` needs a live `valid`/`errors`/`warnings` verdict to show on the Overview tab, computed via the same `_validate_one()` the persisting `validate_tools()` wrapper calls. Using `validate_tools()` directly there would have stamped `last_validated_at` on every single `GET /tools/{name}`, making "last validated" meaningless (it would just track "last viewed"). `_to_detail()` calls `_validate_one()` directly instead — display-only, no persistence — and only `validate_tools()` (called from the explicit `POST /tools/{name}/validate` / bulk `POST /tools/validate` routes) stamps the timestamp. Verified by a dedicated test asserting two consecutive `GET`s produce an unchanged `last_validated_at`.

## Phase 11 — Nikto & Nuclei Integration

### Each plugin's `ScanProfile`/`AdvancedOptions` model has genuinely different fields, by design

Nmap's profile model has `required_ports`/`required_scripts`/`script_args`; Nikto's has `tuning`/`plugins`/`timeout_seconds`; Nuclei's has `templates`/`tags`/`exclude_tags`/`severities`. These were kept as three distinct Pydantic models (one per plugin, same pattern Nmap already established) rather than forced into one shared model with every possible field, because each tool's tunable dimensions are genuinely different concepts, not different names for the same thing — a shared model would either need every field optional (making validation meaningless — nothing would ever be "required" per tool) or a lowest-common-denominator subset that silently drops what makes each tool's profile system actually useful (e.g. Nuclei's severity filter has no Nmap equivalent to unify it with). The commonality that *does* exist (id/name/description/category/icon/supported_targets/risk_level/estimated_duration/built_in/minimum_tool_version) is already identical across all three by convention, without a shared base class forcing it.

### ...but the shared HTTP schema (`ScanProfileRead`/`ScanProfileWrite`) must be one superset schema, and this was a real bug until it was

The plugin-level models being intentionally distinct doesn't mean the *HTTP* layer gets to be distinct too — `/tools/{tool_name}/profiles` is one shared endpoint/schema across every profile-supporting tool, by original design (`backend/services/scan_profile_service.py`'s docstring already said "already shaped for a second scanner plugin to adopt the same pattern later" back when only Nmap had one). The schema just hadn't actually been generalized yet, because nothing had exercised the gap: `ScanProfileRead`/`ScanProfileWrite` had Nmap's field names hardcoded (`required_ports` etc., required, not optional), and `_to_read` accessed them via plain attribute access (`profile.required_ports`). The moment Nikto/Nuclei's own, differently-shaped `ScanProfile` instances flowed through the same `_to_read`, this raised a real `AttributeError: 'ScanProfile' object has no attribute 'required_ports'` — a 500, not a validation error, since it happened while *constructing* the response, not while parsing a request. Fixed by making the schema a superset (every tool's fields, all optional with empty/None defaults) and `_to_read` read each one via `getattr(profile, name, default)`. This is the correct generalization *because* the HTTP contract is genuinely meant to be shared (one route, one frontend Profiles tab for any tool), whereas the plugin-internal models correctly stay separate (see above) because their fields are genuinely tool-specific. Caught only by calling the real HTTP endpoint after discovery already reported `supports_profiles: true` for both new plugins — no unit test of `ProfileManager` directly (which talks to the plugin's own model, not this schema) could have caught it, consistent with this project's repeated experience that a schema/API-boundary bug needs a real HTTP-level check, not just a plugin-internal one.

### `minimum_nmap_version` renamed to `minimum_tool_version` project-wide

Found while designing Nikto/Nuclei's own profile models: the "generic" Scan Profile schema had an Nmap-specific field name (`minimum_nmap_version`) baked in from when Nmap was the only tool with profiles — the exact same class of problem as the `ScanProfileRead` bug above, caught proactively this time instead of via a 500. Renamed everywhere it appeared: `backend/plugins/plugins/nmap/models.py`, all 29 of Nmap's built-in profile JSON files, `backend/schemas/scan_profile.py`, `backend/services/scan_profile_service.py`, and the frontend's `types/scan-profile.ts`/`ToolProfilesPanel.tsx`. No test referenced the old name directly (confirmed via a repo-wide grep before renaming), so this was a pure, safe rename — re-verified by the full backend suite passing unchanged afterward.

### Cross-tool correlation branches on `Observation.plugin`, which already existed but had never been used for this

`Observation.plugin` (the originating tool name, e.g. `"nmap"`/`"nikto"`/`"nuclei"`) has been set at persist time since `HostInventoryService` was built (Phase 8/9) — every observation already knows which tool produced it. No existing correlation rule (Phase 9's 23, or Phase 10's additions) ever read it; every rule filtered only on `observation.source` (the script/check id string) since, until this phase, no rule's *point* was to compare across tools. `cross_tool_rules.py` is the first to make that comparison the whole premise — `MultiToolConfirmedCveRule` groups observations by an extracted CVE id and checks `len({obs.plugin for obs in group}) >= 2`; `KnownVulnerableTechnologyConfirmedByTemplateRule` explicitly filters for `observation.plugin == "nuclei"` (the live confirmation) versus everything else (potential supporting corroboration). No engine change was needed for this — `RuleContext.observations` already contained everything required; this phase is purely two new rules using a field that was already there.

### A known-vulnerable version table exists but is never, by itself, sufficient to name a CVE in a Finding

`_KNOWN_VULNERABLE_VERSIONS` (a tiny, static `(product, version_prefix, CVE)` table, currently just the Apache 2.4.49/2.4.50 path-traversal CVEs from the phase brief's own worked example) exists purely to *recognize and label* which known pattern a version match corresponds to — it is never, by itself, treated as sufficient evidence to cite that CVE in a persisted Finding. `KnownVulnerableTechnologyConfirmedByTemplateRule.evaluate()` returns zero candidates for a version match with no corroborating Nuclei observation actually confirming that exact CVE live against the host (verified directly by `test_known_vulnerable_technology_rule_requires_live_nuclei_confirmation`). This distinction matters because `technology_rules.py`'s pre-existing `EndOfLifeTechnologyRule` explicitly documents *never* citing a CVE from a version-table lookup alone ("never a guessed CVE number for a specific version") — this new rule doesn't violate that principle, it adds a second, independent evidence requirement (a live template match) on top of the version-table recognition, so the actual claim in the Finding is always backed by something Nuclei itself, live, actually observed, never a static table's assertion alone.

### `FindingCandidate.title_override` existed since Phase 9 but was dead code until this phase

Declared on the dataclass from the start, `title_override` was never read by `correlation_service.py`'s `_upsert_finding` — every Finding's `title` came from `rule.title` (the rule's fixed, static class attribute) unconditionally. Invisible until now because no rule had a reason to want a per-match title; `cross_tool_rules.py` is the first where the generic rule title ("Same CVE Independently Reported by Multiple Tools") is much less useful than a match-specific one ("CVE-2021-41773 corroborated by 2 tools"). Fixed by using the first candidate's `title_override` when a Finding is created (existing rows keep their original title on re-correlation, matching how `description`/`impact` are also only ever set once, at creation). Deliberately *not* re-evaluated on every re-correlation pass — a rule matching multiple distinct sub-cases on one host still collapses onto the one Finding row per the Phase 9 fingerprint design (`rule_id` + `host.fingerprint>`, not per-candidate), so there is exactly one title slot, filled once, the same way there has only ever been one description/impact slot.

### Real execution against Nikto/Nuclei was not demonstrated in this session — an honest gap, not a fabricated pass

Neither tool is installed on this Windows development machine (`shutil.which("nikto")`/`shutil.which("nuclei")` both return `None`; no Go toolchain available to `go install` Nuclei as a workaround). Installing either here would also cut against the project's own established Linux-only-runtime workflow (`.claude/CLAUDE.md`) — these tools' real home is the Linux machine this code gets copied to after each phase, not this dev box. Rather than skip verification silently or fabricate a "ran successfully" claim, both plugins' parsers/normalizers were verified against realistic sample output matching each tool's real, documented XML/JSONL schema (including a byte-for-byte plausible CVE-2021-41773 JSONL line matching Nuclei's actual output format), and a `@pytest.mark.skipif`-guarded real-execution test was written for each (identical convention to `test_nmap_plugin.py`'s own real-execution test), which will run for the first time once this code reaches a Linux machine with these tools installed.

## UI/UX Refactor — Remove Internal Data Model Pages

### "Security Overview" was dropped from the nav array but not deleted — the two instructions genuinely conflicted, and the letter of both was honored

The removal list named exactly seven paths (`/hosts`, `/host-overview`, `/services`, `/technologies`, `/technology-overview`, `/observations`, `/operating-systems`) — `/security-overview` was conspicuously absent. But the same brief separately states the nav should contain *only* Dashboard/Assessments/Tools/Executions/Findings/Reports/Settings, "nothing else." Interpreted literally, both instructions can't be simultaneously satisfied by leaving Security Overview exactly as it was. Resolved by treating them as addressing two different things: the *navigation array* must have exactly seven entries (so Security Overview's nav-items row was removed), while the *route and page* were not touched (so a page not named in the explicit removal list wasn't deleted out from under a design that might still want it). A new "Security Overview" button was added to the Findings page header specifically so the route doesn't become an orphaned, unreachable dead page — satisfying the cleanup section's "remove... unused navigation links" without also making a live, non-duplicate page (severity distribution + critical-findings tiles, content nothing else on Findings/Dashboard shows) permanently unreachable.

### The former `/hosts/:id` route's content was extracted into a component, not deleted with the page

`HostDetails.tsx` (routed, `/hosts/:id`) contained the only real UI for a host's full nested detail (network interfaces/services/technologies/OS/observations/execution history) — deleting it outright would have been a genuine information-architecture regression, not a cleanup, since nothing else in the app could show that data. The brief's own "Move Information" section is explicit that this content should become "contextual drill-down information," not disappear — so its JSX was extracted near-verbatim into `HostDetailPanel.tsx` (same tabs, same fields, same empty states) and re-parented under the Assessment Details "Assets Discovered" tab, selected via a callback instead of a URL. `HostTable.tsx` was changed from an unconditional `navigate(/hosts/:id)` on row click to an `onSelectHost` prop specifically so it carries no assumption about what "selecting a host" leads to — today that's inline expansion in one place; a table component that hardcodes navigation can only ever be reused by duplicating it, which is exactly what this whole refactor is trying to stop doing.

### Search deep-linking into an Assessment required one real, additive backend field — this was not achievable as a pure frontend change

The brief requires "Search results should deep-link into the relevant Assessment, Execution, or Finding" instead of navigating to dedicated per-resource pages. `SearchResult` (`backend/schemas/host_inventory.py`) carried `host_id` but never `assessment_id` — and for service/technology/observation rows specifically, `assessment_id` isn't even a column on those tables (only `host_id` is; the assessment relationship is one hop away, through `DiscoveredHost`). Two options existed: (a) have the frontend make a second API call per search-result click to resolve `host_id -> assessment_id` before navigating, or (b) add the field to the existing `search()` query via a join, which the backend can do in the same query it already runs. (b) is strictly better — no added latency, no added round-trip, and it's additive (a new nullable field on an existing schema, not a removed or altered one), so it doesn't conflict with "do not delete backend functionality." `host_owned_by_active_assessment()` (the existing soft-delete-aware filter) was left untouched; the new `assessment_id` projection is a separate `select(Model, DiscoveredHost.assessment_id).join(...)` addition alongside it.

### Dashboard's inventory widgets (host/service/tech distribution charts, "Newest hosts") were removed, not merely relinked

The brief's Dashboard section is a positive list ("Assessments, Executions, Findings, Critical Findings, Reports, Tool Status, Recent Activity") immediately followed by "Do not expose internal inventory pages." The pre-existing "Host dashboard" section (OS distribution, technology distribution, top-open-ports, service-distribution charts, "Newest hosts" list) is exactly the kind of internal-inventory content that instruction rules out — relinking its "Newest hosts" card to an assessment instead of the deleted `/hosts` page would have kept the letter of "no broken links" while still violating the Dashboard's own content rule. Removed the section entirely instead, along with the four inventory-count stat tiles (Hosts/Services/Technologies/Observations) from the overview row and two inventory-growth trend charts (Host growth, Observation trend) — keeping only the four assessment-workflow tiles (Assessments/Targets/Findings/Reports) and the two assessment-workflow trend charts (Finding trend, Execution timeline) the brief's own list actually asks for. "Tool status" and "Recent activity" (both explicitly requested, neither previously present) were added using data the app already had (`useTools()`, `useJobs()`) rather than any new backend endpoint.

## Execution Engine Fix — Persist Partial Output on Non-Zero Exit

### A non-zero exit code is not the same claim as "nothing real was collected"

`ExecutionManager._execute_job` (`backend/workers/manager.py`) called `_persist_scan_results` — the one path that writes a `RawToolOutput` row and attempts `parse()`/`normalize()` — only `if succeeded` (`raw_output.exit_code == 0`). This conflates two independent facts: whether the *process* exited cleanly, and whether the *process produced real, usable output before it stopped*. A tool can do both, one, or neither — Nikto hitting its own internal per-host error-rate limit partway through an SSL-heavy scan is a real, documented example: an unrelated target-side TLS handshake incompatibility causes Nikto to abort and exit 1, but it has already written a fully valid XML report covering every request that did succeed before that point. Gating persistence on exit code alone silently threw that away — the raw stdout/stderr files still landed on disk (a separate, unconditional code path a few lines earlier), but nothing else about it was ever recorded: no `RawToolOutput` row, so `GET /jobs/{id}/raw-output` 404s even though real data exists in the job's own output directory; no attempt at `normalize()`, so even a plugin capable of extracting partial inventory from a partial report never got the chance.

Per `.claude/CLAUDE.md`'s "only display real collected data": discarding real, already-collected data is a *worse* violation of that rule than persisting it under a job still honestly marked `FAILED` with its real exit code and error message. The rule protects against fabrication, not against surfacing something real just because it arrived alongside a non-zero exit.

Fixed by changing the gate to `if raw_output.stdout.strip():` — persistence now tracks "did the tool say anything," not "did the tool exit 0." The job's terminal status (`COMPLETED`/`FAILED`) is computed identically before and after this change, straight from the real exit code — this only changes whether real, already-produced output is recorded, never whether a job is reported as having succeeded. `_persist_scan_results`'s pre-existing try/except around `parse()`/`normalize()` (a Phase 7 guarantee — a parsing hiccup logs and continues rather than raising) already made this call safe regardless of the job's own status, so no new error handling was needed to support calling it from a FAILED path.

### Regression tests use a third dummy-plugin failure mode, not a real tool

`backend/plugins/plugins/dummy_execution/plugin.py` gained `"fail-with-output"` (exit code 1, real non-empty stdout) alongside its pre-existing `"fail"` (exit code 1, empty stdout) and `"raise"` modes — deliberately keeping the execution-engine test suite's existing convention (Phase 6's explicit instruction: verify the engine via `DummyExecutionPlugin`, never a real tool) rather than reaching for a real Nikto/Nuclei run neither of which is installed on this Windows dev machine. Two new tests in `test_executions.py` lock in both halves of the fixed behavior: `test_failed_job_with_partial_output_still_persists_raw_output` (failed job, real stdout -> `GET /jobs/{id}/raw-output` returns 200 with the real content) and `test_failed_job_with_no_output_records_nothing` (failed job, empty stdout -> still 404s, proving the fix didn't just remove the check entirely). Full suite re-verified at 231 passing, 2 skipped (unchanged skip count — the pre-existing Nikto/Nuclei real-execution skips) after this change.

## Phase 12 — Assessment Pipeline & Scanner Orchestration + Intelligent Standard Nmap Profile

### Reusing `Target`/`ToolExecution`/`ExecutionPlanner` for pipeline-generated scans, instead of a parallel execution path

The brief needs the platform to run Nikto/Nuclei against endpoints *it itself generated* (e.g. `http://host:80`), not the user's original target. Two shapes were possible: (a) invent a parallel "run this tool against this raw string" path that bypasses `Target` entirely, or (b) make the generated endpoint a real `Target` row and drive it through the exact same `ExecutionPlanner`/`ExecutionManager` every manual execution already uses. (b) was chosen specifically because `ExecutionPlanner._plan_one` already contains every "should this actually run" judgment call this project has accumulated over five phases — plugin not registered, disabled, unhealthy, can't validate this target type — each producing the right `SKIPPED` status and message. Reimplementing any slice of that inside the Pipeline Engine would have been duplicated logic that could silently drift from the real planner's behavior. The cost of this approach is two additive columns on `Target` (`origin`, `discovered_from_execution_id`) so the user-facing Targets tab/picker and `Assessment.target_count`/the Dashboard's `targets` stat can filter these synthetic rows back out (see the real bug below on that filtering being incomplete in the first pass) — a small, one-time cost against reusing an entire, already-hardened planning pipeline.

### The Pipeline/Correlation stage boundary is a fixed 3-tier enum (`recon`/`scan`/`correlate`), not a generic dependency graph

`PipelineJob.stage` is a `PipelineStage` enum with exactly three values, and `PipelineJob` has no `depends_on_job_id`/parent-pointer column at all. This was a deliberate rejection of a more "general" graph model: the Assessment Pipeline's actual shape, per the brief's own execution-chain diagram, is a fixed pipeline (Nmap → decision → follow-up scanners → Correlation), never an arbitrary DAG a user assembles from parts. A generic dependency-graph schema would be solving a problem this phase doesn't have, at the cost of real complexity (topological ordering, cycle detection, a graph-layout algorithm on the frontend) for zero present benefit — and the frontend confirmed no DAG-rendering library (react-flow, dagre, etc.) exists anywhere in this repo, so a fixed 3-column layout needed none either. If a future phase adds genuinely branching, user-configurable pipelines, this is the point to revisit; today, `stage` plus "which host/service this scan-stage row is about" is sufficient to reconstruct the exact graph the brief asks the frontend to render.

### `PipelineRuleRegistry.decide()` returns the *first* match, and rule ordering is a real, load-bearing contract — not an implementation detail

Unlike `CorrelationRuleRegistry` (which evaluates *every* rule and merges all matches, since multiple correlation rules legitimately co-apply to the same host), exactly one `PipelineRule` should ever claim a given service — a port is either HTTP, HTTPS, SSH, SMB, a database, or unrecognized, never two of those at once. `backend/pipeline/rules/__init__.py`'s `ALL_RULES` ordering (`web_rules` before `reserved_rules`, and within `web_rules`, HTTPS before HTTP) is therefore not cosmetic: `HttpsServiceRule` must be asked before `HttpServiceRule` so a port correctly recognized as HTTPS by *port number* (443/8443) never falls through to the HTTP rule's own, separate service-name heuristic. This ordering dependency is documented directly in both the registry and the rules' own docstrings/comments, and is additionally hardened (not just documented) by the fix below, which makes `HttpServiceRule` correct even if something ever called it standalone, outside the registry's ordering.

### `SkipDecision` carries its own `rule_id`/`category`, rather than the engine inferring them

Early in this phase, `_record_skip`'s `Observation` needed a `category` (`ObservationCategory`) and a stable dedup key — but the engine has no principled way to derive "this skip was about SSH, so the category is AUTH" from a plain reason string. Rather than pattern-matching the reason text (fragile, and a layering violation — the engine would be re-deriving something only the rule itself actually knows), each reserved rule sets its own `category`/`rule_id` directly on the `SkipDecision` it returns, the same way a `CorrelationRule` declares its own `category` as class-level metadata. The engine only ever consumes what the rule already decided, never infers it.

### Real bug: `AmbiguousForeignKeysError` from a second, opposite-direction FK between `targets` and `tool_executions`

Adding `Target.discovered_from_execution_id` (FK → `tool_executions.id`, "which recon execution generated this synthetic target") gave the `targets`/`tool_executions` table pair a *second* foreign-key path, in the opposite direction from the pre-existing `ToolExecution.target_id` (FK → `targets.id`). SQLAlchemy's relationship auto-join logic walks *all* FK constraints between two tables when configuring a relationship between their models — with two now present, it could no longer infer, on its own, which one `Target.tool_executions` or `ToolExecution.target` should actually join on, and raised `AmbiguousForeignKeysError` at first model-registry configuration (i.e. import time, not query time — every single test in the suite failed, 89 on the first run, since `backend.models` is imported by `conftest.py` before anything else). Fixed by adding explicit `foreign_keys=` to both relationship declarations, each pointing at the one column that relationship actually means. This is a completely mechanical fix once diagnosed, but the failure mode itself (an innocuous-looking new nullable FK column breaking dozens of unrelated tests with a mapper-configuration error, not a migration error) is exactly the kind of thing "run the full suite after any model change, not just the new tests" exists to catch — confirmed directly here, since `alembic upgrade head` itself succeeded cleanly; only the ORM's relationship configuration broke.

### Real bug: the `intelligent_standard` profile's first cut was too slow to be a practical one-click default

The brief's "Standard Profile" section lists "Aggressive detection options suitable for assessment" as an explicit requirement, alongside "Full TCP port scan." The first draft satisfied the full-port-scan requirement (`-p 1-65535`) and reached for `--version-intensity 9` (Nmap's own maximum) to satisfy "comprehensive," but never added a `-T` timing template at all — meaning Nmap silently used its own default, `-T3` (normal), the slowest of Nmap's five built-in templates short of the deliberately-paranoid ones. Verified live: a real full-port scan against `127.0.0.1` on this dev machine was still running after 5+ minutes and had to be killed. This is a real instance of technically satisfying a checklist ("comprehensive") while missing the checklist's own adjacent, explicit requirement ("suitable for assessment," i.e. actually fast enough to use as a default). Fixed by adding `-T4` (Nmap's own "aggressive" timing template — safe, not "intrusive"; it only shortens timeouts and increases parallelism, changing nothing about *what* gets probed) and dropping `--version-intensity` back to Nmap's own default (7, already thorough) rather than the maximum (9). The identical scan shape then completed in 209 seconds. This was only ever going to be caught by actually running it — nothing about the profile JSON itself signals "too slow."

### Real bug: the execution graph showed "Waiting" for a job's entire real running duration

`PipelineJob.status` is stamped once at creation (from the freshly-planned `ToolExecution`'s initial status, always `PENDING`/`WAITING` at that point) and then only re-stamped by `backend.workers.pipeline_hook.on_job_terminal` — which, by design, only fires on a job's *terminal* transition (completed/failed/cancelled/timeout/skipped), since that's the only transition the Pipeline Engine needs to act on (schedule the next stage, or finalize). Nothing re-stamps the row on the ordinary, non-terminal PREPARING→RUNNING transition every job goes through. Caught live, not by any test written before this point: the real Nmap recon node sat at "Waiting" in the browser the entire ~3.5 minutes its real scan actually ran, only flipping to "Completed" once it finished — a real, user-visible inaccuracy, not a cosmetic nitpick, since "is anything actually happening right now" is the whole point of an execution graph. Fixed in `PipelineService._to_read()` (the read/display path only — the persisted column and `try_finalize()`'s own terminal-check logic are untouched and remain correct): for any job still snapshotted at `WAITING`, look up its underlying `ToolExecution`'s *current* status and re-derive the display status live via the same `EXECUTION_STATUS_TO_PIPELINE_JOB_STATUS` mapping the engine itself uses at creation/terminal time (moved to `backend/pipeline/models.py` specifically so both modules share one mapping instead of two copies drifting apart). Locked in by `test_get_pipeline_shows_live_running_status_before_the_job_terminates`, and re-confirmed live via a second Playwright pass after the fix — the recon node correctly showed "Running" the whole scan.

### Real bug: `Assessment.target_count`/the Dashboard's `targets` stat counted synthetic pipeline targets too

`AssessmentService._read()`'s and `_count_targets_bulk()`'s target-count queries, and `DashboardService._overview()`'s own `targets` count, all counted every `Target` row unconditionally — including the Pipeline Engine's synthetic endpoint targets, which the Targets tab's own *list* query (`TargetService.list()`/`list_all()`) was already correctly filtered to exclude (`origin == "user"`). The result, caught live: after one real pipeline run generated four synthetic endpoint targets on top of one real user target, the Targets tab's own header read "Targets (5)" over a table that — correctly — showed only 1 row underneath it. A visible, confusing inconsistency between a count and the list it's supposed to describe. Fixed by adding the identical `origin == "user"` filter to all three counting queries, so every place this project surfaces "how many targets does this assessment have" agrees with the one place that actually lists them. `test_pipeline_generated_targets_are_excluded_from_assessment_target_count` locks this in end-to-end (seeds two real synthetic targets via `PipelineEngine.advance_after_nmap`, asserts three rows exist in the DB but `target_count` still reads 1).

### Real bug (caught by the test suite, never reached a live run): `HttpServiceRule`'s service-name heuristic matched `"https"` too

`contains_any(name, ("http", "www"))` is a plain substring check, and `"http"` is a substring of `"https"` — so `HttpServiceRule.evaluate()` called directly (bypassing `PipelineRuleRegistry`'s own https-first ordering, which normally prevents this in practice) matched a service literally named `"https"` too. `test_https_rule_fires_before_http_rule_for_port_443` — written to assert the *opposite*, that `HttpServiceRule` correctly declines an HTTPS-named service — caught this immediately, before any live testing. Fixed by explicitly checking for and excluding the HTTPS hints first inside `HttpServiceRule` itself, so the rule is correct standalone, on its own merits, rather than merely correct-by-registry-ordering (which was already true, and remains the primary real-world guarantee, but a rule that's only correct because of where it happens to sit in a list is a latent bug waiting for the next reordering or a second call site).

### Real bug (also caught by the test suite): a `SkipDecision` and a "no web service" skip could both fire for the same host

The first draft of `_advance_host` tracked a single `any_web_service` flag, set only when a `ScheduleDecision` fired, and fell through to a generic "No supported web services discovered." skip whenever it stayed `False` — which included every host where a `SkipDecision` (SSH/SMB/database) *did* fire, since that path never set the flag. `test_advance_after_nmap_records_observation_and_skip_for_ssh_only_host` caught the resulting double-skip (an SSH-only host got both the correct SSH-specific message and a second, redundant generic one, four `PipelineJob` rows instead of two). Fixed by renaming and broadening the tracked condition to `any_recognized_service` — true whenever *any* rule returned *any* decision (schedule or skip), so the generic fallback is reserved for the case the brief's own worked example actually describes: a host where literally nothing was recognized at all.

**Honest scope note**: this was found and fixed by reading `_execute_job` closely (a persistence gate keyed on exit code is wrong by construction, independent of which tool eventually exercises it), not by reproducing the Nikto scenario live — neither Nikto nor Nuclei has been executed for real in this repository yet (see Phase 11's own testing note). The Nikto error-rate-limit example is real, documented Nikto behavior used to justify *why* the gate was wrong, not a claim that this specific scenario was reproduced here.

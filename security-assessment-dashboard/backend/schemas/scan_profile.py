...
Nikto, Nuclei so far), so each plugin's own field set (e.g. Nmap's
``required_ports``/``required_scripts``, Nikto's ``tuning``/``plugins``,
Nuclei's ``templates``/``tags``/``severities``) is represented here as an
optional field that stays ``None``/empty for a tool whose own model
doesn't have it. ``ScanProfileService._to_read`` reads each field via
``getattr`` with a default rather than direct attribute access for
exactly this reason -- the object handed in is one specific plugin's own
``ScanProfile`` instance, not this shared shape.

Adding a fourth profile-supporting plugin with genuinely new tunable
dimensions means adding new optional fields here, never touching what
already exists for Nmap/Nikto/Nuclei.
"""

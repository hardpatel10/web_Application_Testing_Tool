"""Loads, validates, searches, and persists Nmap Scan Profiles.
...
directory (``data/profiles/nmap/`` by default) so a plugin reinstall or
...
class ProfileNotFoundError(PluginNotFoundError):
    ...
    from inside the synthetically-loaded module — caught directly via a
    real 500 while testing profile edit/delete guards.
    """
...
class ProfileManager:
    ...
    def create_custom(self, data: dict) -> ScanProfile:
        """Validate and persist a new custom profile.

        Takes a plain ``dict``, not a pre-built :class:`ScanProfile`
        instance — this plugin's directory is loaded as a synthetic
        package with its own fresh module identity (see the Phase 4
        loader), so a caller outside this package (``ScanProfileService``)
        cannot construct a `ScanProfile` that is safely interchangeable
        with this module's own. Validating the dict here, against this
        module's own ``ScanProfile``, sidesteps that entirely.
        """

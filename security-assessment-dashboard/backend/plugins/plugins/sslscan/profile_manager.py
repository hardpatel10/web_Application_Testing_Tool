"""Loads, validates, searches, and persists SSLScan Scan Profiles.
...
directory (``data/profiles/sslscan/`` by default) so a plugin reinstall or
...
Structurally identical to ``backend.plugins.plugins.nuclei.profile_manager``
and ``backend.plugins.plugins.nikto.profile_manager`` (same file-per-
profile-id JSON storage, same built-in/custom precedence, same CRUD
contract) -- kept as a separate class per plugin because each plugin's
``ScanProfile``/``ProfileCategory``/``RiskLevel`` types are deliberately
distinct Pydantic models, and this manager validates against its own
plugin's types only, for the same synthetic-module-identity reason
documented below.
